...
import { getSocketBaseUrl } from "@/lib/services/api-client";
import type {
...
function getOrCreateBrowserClientId(): string {
  try {
    const existing = localStorage.getItem(BROWSER_CLIENT_ID_STORAGE_KEY);
    if (existing && existing.length >= 8) {
      return existing;
    }
    const created = createBrowserClientId();
    localStorage.setItem(BROWSER_CLIENT_ID_STORAGE_KEY, created);
    return created;
  } catch {
...
export default function PlayPage() {
  ...
  const browserClientIdRef = useRef<string>("");
  const playerIdentityRef = useRef<{ playerId: number | null; nickname: string }>(
    { playerId: null, nickname: "" }
  );
  ...
  const getBrowserClientId = useCallback(() => {
    ...
    return browserClientIdRef.current;
  }, []);
  ...
  const emitRejoinFromCache = useCallback((targetSocket: TypedSocket, options?: { force?: boolean }) => {
    try {
      ...
      if (!resolvedPlayerId || !resolvedNickname) {
        const currentIdentity = playerIdentityRef.current;
        if (currentIdentity.playerId && currentIdentity.nickname.trim()) {
          resolvedPlayerId = currentIdentity.playerId;
          resolvedNickname = currentIdentity.nickname.trim();
        }
      }
  ...
  useEffect(() => {
    ...
    s.on("session:admin-disconnected", () => {
      setHostNotice(t("play.hostDisconnected"));
  ...
  const joinGame = (e: React.FormEvent) => {
    ...
    socket.emit("player:join", {
      ...
      browserClientId: getBrowserClientId(),
    });
  };
        ...
        <AnimatePresence>
          {hostNotice && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="mx-auto mb-3 max-w-xl rounded-xl border border-amber-300/40 bg-amber-500/20 px-4 py-3 text-center text-sm font-semibold text-amber-100"
            >
              {hostNotice}
            </motion.div>
          )}
