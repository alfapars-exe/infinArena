...
const QUESTION_NUMERIC_RULES = {
  timeLimitSeconds: { min: 5, max: 120 },
  basePoints: { min: 100, max: 5000 },
  deductionPoints: { min: 0, max: 1000 },
  deductionInterval: { min: 1, max: 60 },
} as const;

type QuestionNumericField = keyof typeof QUESTION_NUMERIC_RULES;

const QUESTION_NUMERIC_FIELDS = Object.keys(
  QUESTION_NUMERIC_RULES
) as QuestionNumericField[];
...
export default function QuizEditor() {
  ...
  useEffect(() => {
    ...
    autoSaveTimer.current = setTimeout(async () => {
      ...
      await authedFetch(`/api/quizzes/${quizId}`, {
        method: "PUT",
  ...
  const addQuestion = async (questionToSave: Question = newQuestion) => {
    if (!questionToSave.questionText.trim()) return;

    const res = await authedFetch(`/api/quizzes/${quizId}/questions`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...questionToSave,
        mediaUrl: questionToSave.mediaUrl || null,
        backgroundUrl: questionToSave.backgroundUrl || null,
      }),
    });
  ...
  const updateQuestion = async (question: Question) => {
    ...
    await authedFetch(`/api/quizzes/${quizId}/questions`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    void downloadAuthedFile(
                      `/api/quizzes/${quizId}/export?format=excel`,
                      `quiz-${quizId}.xlsx`
                    );
                  }}
                  className="block w-full text-left px-4 py-2 text-sm text-white hover:bg-white/10 rounded-t-lg bg-transparent border-0"
                >
                  Excel (.xlsx)
                </button>
                <button
                  ...
                  onClick={() => {
                    setIsExportMenuOpen(false);
                    void downloadAuthedFile(
                      `/api/quizzes/${quizId}/export?format=word`,
                      `quiz-${quizId}.docx`
                    );
                  }}
                  className="block w-full text-left px-4 py-2 text-sm text-white hover:bg-white/10 rounded-b-lg bg-transparent border-0"
                >
                  Word (.docx)
                </button>
              </div>
...
function QuestionModal({
  ...
  const [numericDrafts, setNumericDrafts] = useState<
    Record<QuestionNumericField, string>
  >({
    timeLimitSeconds: String(question.timeLimitSeconds ?? ""),
    basePoints: String(question.basePoints ?? ""),
    deductionPoints: String(question.deductionPoints ?? ""),
    deductionInterval: String(question.deductionInterval ?? ""),
  });
  const [numericTouched, setNumericTouched] = useState<
    Record<QuestionNumericField, boolean>
  >({
    timeLimitSeconds: false,
    basePoints: false,
    deductionPoints: false,
    deductionInterval: false,
  });
  ...
  const updateChoice = <K extends keyof Choice>(index: number, field: K, value: Choice[K]) => {
  ...
  const updateNumericDraft = (field: QuestionNumericField, value: string) => {
    setNumericDrafts((prev) => ({ ...prev, [field]: value }));
    const rule = QUESTION_NUMERIC_RULES[field];
    const parsed = parseBoundedInteger(value, rule.min, rule.max);
    if (parsed !== null && question[field] !== parsed) {
      onChange({ ...question, [field]: parsed });
    }
  };

  const touchNumericField = (field: QuestionNumericField) => {
    setNumericTouched((prev) => ({ ...prev, [field]: true }));
  };

  const getNumericError = (field: QuestionNumericField): string | null => {
    if (!numericTouched[field]) return null;
    const rule = QUESTION_NUMERIC_RULES[field];
    const parsed = parseBoundedInteger(numericDrafts[field], rule.min, rule.max);
    if (parsed !== null) return null;
    return t("editor.rangeError", { min: rule.min, max: rule.max });
  };

  const allNumericDraftsValid = QUESTION_NUMERIC_FIELDS.every((field) => {
    const rule = QUESTION_NUMERIC_RULES[field];
    return parseBoundedInteger(numericDrafts[field], rule.min, rule.max) !== null;
  });

  const buildQuestionWithNumericDrafts = (): Question | null => {
    const updates: Partial<Question> = {};

    for (const field of QUESTION_NUMERIC_FIELDS) {
      const rule = QUESTION_NUMERIC_RULES[field];
      const parsed = parseBoundedInteger(numericDrafts[field], rule.min, rule.max);
      if (parsed === null) {
        return null;
      }
      updates[field] = parsed as Question[typeof field];
    }
  ...
  const handleSaveClick = () => {
    if (!allNumericDraftsValid) {
      setNumericTouched({
        timeLimitSeconds: true,
        basePoints: true,
        deductionPoints: true,
        deductionInterval: true,
      });
      return;
    }

    const finalizedQuestion = buildQuestionWithNumericDrafts();
    if (!finalizedQuestion) return;

    onChange(finalizedQuestion);
    onSave(finalizedQuestion);
  };
        ...
        <div className="modal-body-scroll px-4 px-md-6 py-4 space-y-4">
          ...
          <div className="row g-3">
            ...
            <div className="col-12 col-md-6">
              ...
              <input
                type="number"
                min={QUESTION_NUMERIC_RULES.timeLimitSeconds.min}
                max={120}
                value={numericDrafts.timeLimitSeconds}
                onChange={(e) =>
          ...
          <div className="row g-3">
            ...
            <div className="col-12 col-md-4">
              ...
              <input
                ...
                min={0}
                max={1000}
                value={numericDrafts.deductionPoints}
                onChange={(e) =>
            ...
            <div className="col-12 col-md-4">
              ...
              <input
                ...
                min={1}
                max={60}
                value={numericDrafts.deductionInterval}
                onChange={(e) =>
